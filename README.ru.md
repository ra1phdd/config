...
### Merge секретов в срезах структур

Элемент `.security.yml` сопоставляется с элементом основного конфига по обычным scalar-полям, присутствующим в overlay (`name`, `id`, `type` и т. п.). Однозначное совпадение обновляет только защищённые ветки; остальные поля структуры и другие элементы среза сохраняются. Если совпадений нет, добавляется новый элемент. Конфликтующие или неоднозначные селекторы возвращают ошибку.

Поле ключа можно указать явно; несколько полей образуют составной ключ:
...
Все части явного составного ключа обязательны в соответствующем элементе `.security.yml`. Поддерживаются срезы значений и указателей. Неподходящий элемент добавляется в срез, но не записывается в случайную позицию; fixed array расширить нельзя, поэтому загрузка вернёт ошибку.

## Файловые ссылки для структурированных данных

### Каталог типизированных mini-config

`FileMap[T]` подключает каталог как часть общей структуры конфига. Логический ключ становится именем файла без расширения:
...
Файл пользователя `123456` хранится в `users/123456.json`. При загрузке весь каталог декодируется eagerly; один битый entry возвращает ошибку для всего конфига. Доступны `Get`, `All`, `Keys`, `Len`, а `Add`, `Update`, `Set`, `Delete` и `Reload` синхронизированы с диском. CRUD сначала атомарно меняет файл и лишь затем память. `Save` записывает известные элементы, но не удаляет orphan-файлы.

Ключи не могут содержать расширение, разделители пути, `..`, абсолютные/drive/UNC-пути. `FileMap` потокобезопасен внутри одного процесса, но не предоставляет межпроцессную блокировку. Вложенные `FileMap` и mini-config с `SecureString` пока запрещены.
